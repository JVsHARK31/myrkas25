-- =====================================================
-- SUPABASE FINAL SETUP FOR R-KAS APPLICATION
...
DROP POLICY IF EXISTS "Allow read access for authenticated users" ON public.student_grades;
DROP POLICY IF EXISTS "Allow write access for admin users" ON public.student_grades;

DROP POLICY IF EXISTS "Allow read access for authenticated users" ON public.inventory_items;
DROP POLICY IF EXISTS "Allow write access for admin users" ON public.inventory_items;

DROP POLICY IF EXISTS "Allow read access for authenticated users" ON public.events;
DROP POLICY IF EXISTS "Allow write access for admin users" ON public.events;

DROP POLICY IF EXISTS "Allow read access for authenticated users" ON public.documents;
...
CREATE POLICY "Allow read access for authenticated users" ON public.inventory_transactions FOR SELECT USING (auth.role() = 'authenticated');
CREATE POLICY "Allow write access for admin users" ON public.inventory_transactions FOR ALL USING (
    EXISTS (SELECT 1 FROM public.user_profiles WHERE auth_id = auth.uid() AND role = 'admin')
);
...
CREATE POLICY "Allow read access for authenticated users" ON public.event_participants FOR SELECT USING (auth.role() = 'authenticated');
CREATE POLICY "Allow write access for admin users" ON public.event_participants FOR ALL USING (
    EXISTS (SELECT 1 FROM public.user_profiles WHERE auth_id = auth.uid() AND role = 'admin')
);
...
-- Notification policies (users can see their own)
CREATE POLICY "Users can view own notifications" ON public.notifications FOR SELECT USING (user_id = (SELECT id FROM public.user_profiles WHERE auth_id = auth.uid()));
CREATE POLICY "Users can update own notifications" ON public.notifications FOR UPDATE USING (user_id = (SELECT id FROM public.user_profiles WHERE auth_id = auth.uid()));
...
CREATE POLICY "Allow write access for admin users" ON public.activities FOR ALL USING (
    EXISTS (SELECT 1 FROM public.user_profiles WHERE auth_id = auth.uid() AND role = 'admin')
);

-- System settings policies (admin only)
CREATE POLICY "Allow read access for authenticated users" ON public.system_settings FOR SELECT USING (auth.role() = 'authenticated');
CREATE POLICY "Allow write access for admin users" ON public.system_settings FOR ALL USING (
    EXISTS (SELECT 1 FROM public.user_profiles WHERE auth_id = auth.uid() AND role = 'admin')
);

-- Audit log policies (admin only)
CREATE POLICY "Allow read access for admin users" ON public.audit_logs FOR SELECT USING (
    EXISTS (SELECT 1 FROM public.user_profiles WHERE auth_id = auth.uid() AND role = 'admin')
);

-- Master data policies (read for authenticated, write for admin)
...
CREATE TRIGGER update_kertas_kerja_perubahan_updated_at BEFORE UPDATE ON public.kertas_kerja_perubahan FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
...
('02', 'Administrasi', 'Bidang Administrasi Sekolah'),
('03', 'Sarana Prasarana', 'Bidang Sarana dan Prasarana')
ON CONFLICT (kode) DO NOTHING;

INSERT INTO public.standar (kode, nama, bidang_id, deskripsi) VALUES
('01.01', 'Kurikulum', (SELECT id FROM public.bidang WHERE kode = '01'), 'Standar Kurikulum'),
('01.02', 'Pembelajaran', (SELECT id FROM public.bidang WHERE kode = '01'), 'Standar Pembelajaran'),
('02.01', 'Tata Usaha', (SELECT id FROM public.bidang WHERE kode = '02'), 'Standar Tata Usaha')
ON CONFLICT (kode) DO NOTHING;

INSERT INTO public.dana (kode, nama, deskripsi) VALUES
('BOS', 'Bantuan Operasional Sekolah', 'Dana BOS dari pemerintah'),
('APBD', 'Anggaran Pendapatan dan Belanja Daerah', 'Dana dari APBD'),
('SWADAYA', 'Swadaya Masyarakat', 'Dana dari swadaya masyarakat')
ON CONFLICT (kode) DO NOTHING;

INSERT INTO public.rekening (kode, nama, level_rekening, deskripsi) VALUES
('5.1.1', 'Belanja Pegawai', 1, 'Belanja untuk pegawai'),
('5.1.2', 'Belanja Barang dan Jasa', 1, 'Belanja barang dan jasa'),
('5.1.3', 'Belanja Modal', 1, 'Belanja modal/investasi')
ON CONFLICT (kode) DO NOTHING;
