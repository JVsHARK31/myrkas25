-- =====================================================
-- SUPABASE SAFE SETUP FOR R-KAS APPLICATION
...
CREATE TABLE IF NOT EXISTS public.standar (
    ...
    kode VARCHAR(20) UNIQUE NOT NULL,
    nama VARCHAR(255) NOT NULL,
    bidang_id BIGINT REFERENCES public.bidang(id),
    deskripsi TEXT,
    is_active BOOLEAN DEFAULT true,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
...
CREATE TABLE IF NOT EXISTS public.giat (
    ...
    kode VARCHAR(20) UNIQUE NOT NULL,
    nama VARCHAR(255) NOT NULL,
    standar_id BIGINT REFERENCES public.standar(id),
    deskripsi TEXT,
    is_active BOOLEAN DEFAULT true,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
...
CREATE TABLE IF NOT EXISTS public.rekening (
    ...
    nama VARCHAR(255) NOT NULL,
    parent_id BIGINT REFERENCES public.rekening(id),
...
CREATE TABLE IF NOT EXISTS public.komponen (
    ...
    kode VARCHAR(20) UNIQUE NOT NULL,
    nama VARCHAR(255) NOT NULL,
    giat_id BIGINT REFERENCES public.giat(id),
    deskripsi TEXT,
    is_active BOOLEAN DEFAULT true,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
...
DROP TRIGGER IF EXISTS update_kertas_kerja_updated_at ON public.kertas_kerja_perubahan;
...
INSERT INTO public.bidang (kode, nama, deskripsi) VALUES
    ('01', 'Urusan Pemerintahan Wajib', 'Bidang urusan pemerintahan yang wajib dilaksanakan'),
    ('02', 'Urusan Pemerintahan Pilihan', 'Bidang urusan pemerintahan yang bersifat pilihan'),
    ('03', 'Fungsi Penunjang', 'Bidang fungsi penunjang urusan pemerintahan')
ON CONFLICT (kode) DO NOTHING;

INSERT INTO public.dana (kode, nama, deskripsi) VALUES
    ('01', 'Dana Desa', 'Dana yang bersumber dari APBN'),
    ('02', 'Alokasi Dana Desa', 'Dana yang dialokasikan dari APBD Kabupaten'),
    ('03', 'Bagi Hasil Pajak dan Retribusi', 'Dana bagi hasil pajak dan retribusi daerah')
ON CONFLICT (kode) DO NOTHING;

INSERT INTO public.rekening (kode, nama, level_rekening, deskripsi) VALUES
    ('5', 'Belanja', 1, 'Kelompok belanja'),
    ('5.1', 'Belanja Operasi', 2, 'Jenis belanja operasi'),
    ('5.1.01', 'Belanja Pegawai', 3, 'Objek belanja pegawai'),
    ('5.1.02', 'Belanja Barang dan Jasa', 3, 'Objek belanja barang dan jasa'),
    ('5.2', 'Belanja Modal', 2, 'Jenis belanja modal'),
    ('5.2.01', 'Belanja Tanah', 3, 'Objek belanja tanah'),
    ('5.2.02', 'Belanja Peralatan dan Mesin', 3, 'Objek belanja peralatan dan mesin')
ON CONFLICT (kode) DO NOTHING;
...
-- Master data summary view
CREATE OR REPLACE VIEW public.master_data_summary AS
SELECT 
    'bidang' as table_name,
    COUNT(*) as total_records,
    COUNT(CASE WHEN is_active = true THEN 1 END) as active_records
FROM public.bidang
UNION ALL
SELECT 
    'standar' as table_name,
    COUNT(*) as total_records,
    COUNT(CASE WHEN is_active = true THEN 1 END) as active_records
FROM public.standar
UNION ALL
...
UNION ALL
SELECT 
    'rekening' as table_name,
    COUNT(*) as total_records,
    COUNT(CASE WHEN is_active = true THEN 1 END) as active_records
FROM public.rekening
UNION ALL
SELECT 
    'komponen' as table_name,
    COUNT(*) as total_records,
    COUNT(CASE WHEN is_active = true THEN 1 END) as active_records
FROM public.komponen;
